...
- Provider/model: `openai / gpt-4o-mini` cho B1–B4a; `gemini / gemini-3.1-flash-lite` cho các run bonus/extension ở B5.
...
> CLI từ `starter_v0/`: `python chat.py --provider openai --version v4`.
> UI: `python ui/server.py --provider openai --version v4`, sau đó mở `http://127.0.0.1:5000/`. Xem [hướng dẫn UI](../ui/README.md).
...
**Lưu ý phiên bản:** Run nhóm 10/10 ở B1/B3 dùng `v4+p894a5fdf181d+tcb2a07133480`. Các phiên chat B4 và run adversarial B4a dùng `v4+p5ce94b364411+tcb2a07133480`, sau một lần sửa prompt. Không coi đây là cùng một bản artifact. `tools.yaml` hiện tại đã thêm tool bảo hành nên chạy lại có thể cho hash và kết quả khác.
...
10 case tự viết (`data/eval_group.json`, dataset_id `day04_v3_helpdesk_group`): 5 single-turn (G01–G05) và 5 multi-turn (G06–G10). Chạy lệnh `python run_eval.py --provider openai --version v4 --suite group --eval-cases data/eval_group.json` — run được dẫn đạt 10/10 PASS, `case_accuracy = 1.0`, `provider_error_cases = 0`, `measured_cases = 10 = total_cases` (`runs/v4_B_group_openai_20260915T203142683813.json`).
...
Chạy thật 5 phiên (11 lượt) với **v4**, `openai / gpt-4o-mini`, ngày 15/09/2026 lúc 20:41 (UTC+07:00). Artifact: `v4+p5ce94b364411+tcb2a07133480`, khác snapshot run nhóm trong `version_log.csv` như ghi chú ở phần B. Lệnh chạy từ `starter_v0/`: `python chat.py --provider openai --model gpt-4o-mini --version v4`.
...
Các run `bonus` ở mục này dùng prompt `pd7b07ca610be` và tools `t70b719f1528f`, khác artifact v4 ở B4/B4a. Kết quả chỉ áp dụng cho run tương ứng.
...
- **Agent có bao giờ tự đoán asset ID hoặc employee ID không?**
  ...
  V0 từng dùng `laptop` và `Sales` thay cho mã hợp lệ. V4 đã hỏi lại khi thiếu mã trong B4, nhưng A06 vẫn dùng nhầm asset ID làm employee ID.
