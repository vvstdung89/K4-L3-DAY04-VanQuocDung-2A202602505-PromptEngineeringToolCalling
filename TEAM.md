...
| --- | --- | --- | --- | --- |
| Nguyễn Đức Thịnh | 2A202602468 | TODO: điền GitHub username | Cải thiện `system_prompt.md`/`tools.yaml` ở v2 (rule "Confirm before write actions" cho `create_ticket`) và v4 (xử lý 2 case fail nhóm tự viết `G05_policy_then_confirmed_ticket`, `G06_multiturn_asset_correction` trong `eval_group.json`: ngoại lệ tự-xác-nhận-trong-câu, rule latest-turn-only, scope argument cho `policy_area`); chạy eval v2/v4 và ghi `version_log.csv`, `REPORT.md` (bảng B1, B3) | Commit `b5e7211` (v2-Thinh), `caf721e` (add report v2 - Thinh), `6a19ddf` (group - Thinh update); `starter_v0/artifacts/system_prompt.md`, `starter_v0/artifacts/tools.yaml`, `starter_v0/artifacts/version_log.csv` (dòng v2, v4), `starter_v0/artifacts/REPORT.md`; run evidence `starter_v0/runs/v2_B_base_openai_20260915T193928048574.json`, `starter_v0/runs/v4_B_group_openai_20260915T203142683813.json` |
